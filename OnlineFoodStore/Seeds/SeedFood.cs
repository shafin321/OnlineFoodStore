...
namespace OnlineFoodStore.Seeds
{
    public class SeedFood : IFood
    {
        ...
        public SeedFood(ICategory category) // constructor
        {
            // _category = category; // injecting by construcotr
            _category = new SeedCategory();// initilze by SeedCategory Class
...
Avocado trees come from Central America and Mexico. They can grow in many places, as long as it is not too cold.",
        ...
        {
            return foodList.FirstOrDefault(food => food.Id == id);
        }

        public IEnumerable<Food> GetFoodsByCategoryId(int categoryId)
        {
            return foodList.Where(food => food.Id == categoryId);
        }
